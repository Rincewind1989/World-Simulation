...
const double WATER_LEVEL = -0.35;		//Highets level Water occurs
...
const double FOOD_TEMPERATURE_FACTOR = 0.75;	//Multiplyfactor for the food growth depending on the temperature
const double FOOD_HEIGHT_FACTOR = 0.5;		//Multiplyfactor for the food growth depending on the height
const double FOOD_NOISE_FACTOR = 5.0;		//Factor for the fluctuation of the food for the perlin noise
const double MAX_FOOD_ON_TILE = 10.0;			//Maximum food that can be on a tile
const double FOOD_GROWTH_FACTOR = 0.01;		//Factor with which the food growths
const double CHANCE_FOOD_GROWTH = 0.2;		//Chance that food growth on a tile per tick
const double CHANCE_BIG_FOOD_GROWTH = 0.005;	//Chance that a big food growth happens on a tile per tick
//----------------------------------------------------------------------
...
const int WINDOW_INFORMATION_HEIGHT = 400;
const double INFORMATION_WINDOW_SPRITE_SIZE = .1;
const int INFORMATION_TEXT_SIZE = 16;
const double RADIUS_NEURON = 3.25;	//Radius of a drawn neuron on the information window
const double DISTANCE_Y_NEURON_FACTOR = 40.0;
const double DISTANCE_INPUT_NEURON_FACTOR = 2.25;;
const double DISTANCE_HIDDEN_NEURON_FACTOR = 4.5;
const double DISTANCE_OUTPUT_NEURON_FACTOR = 7.0;
//----------------------------------------------------------------------
//Life properties
//Organism properties
...
const double ENERGY_HEAT_PRODUCTION = 4.5;		//Factor in which energy is changed into temperature
const double HEAT_LOSS_FACTOR = 0.65;			//Loss of the body temperature towards the environment
const double MOVEMENT_SPEED = 2.0;				//Movementspeed of the organisms
const double MOVEMENT_ENERGY_LOST = 1.05;		//Loss of energy based on the movementspeed
const double CONSUMPTION_FACTOR = 9.0;			//Factor with which the organisms consume food from tile
const double FOOD_ENERGY_FACTOR = 1.35;			//Factor with which food is transformed into energy
const double LOWEST_HEAT_LOSS_VALUE = 0.75;
const double HIGHEST_HEAT_LOSS_VALUE = 1.5;
...
const double SIZE_ENERGY_LOSS_FACTOR = 0.35;	//Loss of energy based on the size factor
const double OPTIM_TEMPERATURE = 32.0;			//Optim temperature the organism needs to work at 100% (Like eating,moving,etc.)
const double MUTATE_STAT = 0.07;				//Chance that a color mutates
const double MOVEMENT_ENERGY_FACTOR_IN_WATER = 1.5;	//Factor for movement energy lost penalty in water
const double MOVEMENT_REDUCTION_IN_WATER = 0.5;
const double TIME_FOR_FITNESS_REPRODUCTION = 1.0;
//----------------------------------------------------------------------																								
//Network properties
...
const int INPUTS_FOR_BODY_PROPERTIES = 7;	//Inputs of the body properties for the neural network(Like bodytemperature, bodyenergy,size, colors ,etc...)
const int NUMBER_NEXT_ENTITES = 4;			//Number of next entities that can be identified
const int NUM_LAYERS = 3;	//Number of layers (This includes input and output layer) MUST ALWAYS BE >= 2
//----------------------------------------------------------------------
//Number of need inputneurons to cover all inputs
const int INPUT_NEURONS = 3 * (2 * ADJACENT_TILES_RADIUS + 1) * (2 * ADJACENT_TILES_RADIUS + 1) //Adjecent tile properties (Like Temperature,Food,Number of entites on tile...)
...
const double CHANCE_MUTATE = 1;	//Chance that a mutation of a weight occurs
const double MUTATION_PERTUBATE_PROCENT = 0.15;	//Max and min pertubation in procent. A real number between this value is taken and added on the weight
